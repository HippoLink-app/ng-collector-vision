...
function buildVersionChain(catalog) {
  ...
  const updateEntries = Object.entries(catalog.updates ?? {})
    .map(([key, update]) => {
      if (!/^(0|[1-9]\d*)$/.test(key) || !isObject(update) || update.to_version !== Number(key)) {
        throw new CatalogV2Error(
          `catalog has an invalid update entry for version ${JSON.stringify(key)}`,
        );
      }
      if (
        ...
        throw new CatalogV2Error(
          `catalog update to version ${key} is not an exact-predecessor delta`,
        );
      }
  ...
  if (
    routeStart > catalog.base.version ||
    updateEntries.at(-1).to_version !== catalog.current_version
  ) {
    throw new CatalogV2Error(
      'catalog retained update route does not connect its base to current_version',
    );
  }
  const versions = [];
  ...
  for (
    let toVersion = catalog.base.version + 1;
    toVersion <= catalog.current_version;
    toVersion += 1
  ) {
