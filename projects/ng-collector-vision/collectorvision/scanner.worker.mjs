...
// ----------------
// main → worker:
//   { type: 'init',   manifest, game, source, assetBase, wasmBase, enableWebGpu,
//                     minCornerConfidence }
...
//                       confidence, cardId, cardName, secondaryId, score,
...
async function fetchWithProgress(url, responseType, expectedTotal, onProgress) {
  ...
  const declaredTotal =
    Number.isSafeInteger(expectedTotal) && expectedTotal > 0 ? expectedTotal : 0;
  let total =
    declaredTotal || Number.parseInt(response.headers.get('content-length') ?? '0', 10) || 0;
...
// Wraps fetch so the catalog client (which buffers whole responses) can report
// download progress. Totals accumulate as each response's Content-Length is seen,
// so the ratio is clamped to be monotonic and held below 1 until the load resolves.
  ...
  return async (...args) => {
    ...
    const counter = new TransformStream({
      transform(chunk, controller) {
        ...
        lastRatio = Math.min(0.99, Math.max(lastRatio, loaded / total));
        onProgress(lastRatio, loaded, total);
...
class WorkerRuntime {
  ...
  async load(onStage) {
    ...
    const detectorBuffer = await fetchBufferCached(
      assetUrl(this.manifest.models[this.detectorConfig.modelKey]),
      detectorVersion,
    ...
    await this.loadCatalog(onStage);
  }

  async loadCatalog(onStage) {
    onStage?.('catalog', 0, 0, 0, false);
    const catalog = await BrowserCatalogV2.forGame(this.game, {
      source: this.source,
      includeMetadata: false,
      fetchImpl: createProgressFetch((ratio, loaded, total) =>
        onStage?.('catalog', ratio, loaded, total, false),
      ),
    });
    const expectedDimensions = this.manifest.catalog?.dims ?? 128;
    ...
    const requestedRows = this.catalogLimit
      ...
      : catalog.rows;
    const records =
      requestedRows < catalog.rows ? catalog.records.slice(0, requestedRows) : catalog.records;

    this.embeddings =
      requestedRows < catalog.rows
    ...
    // Only Scryfall catalogs carry an oracle id; TCGplayer ones have no printing-level grouping key.
    const hasOracleIds = records.some((record) => record.identifiers?.scryfall_oracle);
    this.secondaryIdField = hasOracleIds ? 'scryfallOracleId' : null;
    this.secondaryIds = hasOracleIds
      ? records.map((record) => record.identifiers?.scryfall_oracle ?? null)
      : null;
  ...
  async detect(frameCanvas) {
    ...
    const sharpness =
      sharpnessName && outputs[sharpnessName] ? outputs[sharpnessName].data[0] : null;
